...
def update_output_container(input_year, selected_statistics):
    if selected_statistics == "Recession Period Statistics":
        ...
        # use groupby to create relevant data for plotting
        average_sales = (
            data.groupby("Vehicle_Type")["Automobile_Sales"].mean().reset_index()
        )
        R_chart2 = dcc.Graph(
            figure=px.bar(
                average_sales,
    ...
    # Yearly Statistic Report Plots
        ...
        Y_chart1 = dcc.Graph(
            figure=px.line(
                ...
                x="Year",
                y="Automobile_Sales",
                title="Yearly Automobile sales, period (1980-2023)",
            )
        )
        ...
        Y_chart2 = dcc.Graph(
            figure=px.line(
                ...
                x="Month",
                y="Automobile_Sales",
                title="Total Monthly Automobile sales in the year {}".format(
                    input_year
                ),
            )
        )
        ...
        Y_chart4 = dcc.Graph(
            figure=px.pie(
                ...
                values="Advertising_Expenditure",
                names="Vehicle_Type",
                title="Total Advertisement Expenditure for each Vehicle in the year {}".format(
                    input_year
                ),
            )
        )
